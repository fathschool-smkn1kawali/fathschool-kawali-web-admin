<template>
    ...
    <li class="my_course_section">
        <NavLink :href="route('my-subject')" :active="route().current('my-subject') || route().current('subject.details')">
            <BookOpenIcon class="h-6 w-6" />
            {{ __("My Courses") }}
        </NavLink>
    </li>
    <li class="my_schedule_section">
        <NavLink :href="route('my.schedule')" :active="route().current('my.schedule') || route().current('my.schedule')">
            <CalendarIcon class="h-6 w-6" />
            {{ __("My Schedule") }}
        </NavLink>
    </li>
    <li v-if="can('online-class.index')" class="online_class_section">
        <NavLink :href="route('online.class.index')"
            :active="route().current('online.class.*') || route().current('online.class.*')">
            <PlayCircleIcon class="w-6 h-6" />
            {{ __("Online Classes") }}
        </NavLink>
    </li>
    <li v-if="can('assignment.index')" class="assignment_section">
        <NavLink :href="route('assignment.index')"
            :active="route().current('assignment.*') || route().current('assignment.detail')">
            <ClipboardDocumentListIcon class="h-6 w-6" />
            {{ __("Assignment") }}
        </NavLink>
    </li>
    <li v-if="can('attendance.index')" class="attendance_section">
        <NavLink :href="route('student.attendance.index')" :active="route().current('student.attendance.*')">
            <ShieldCheckIcon class="h-6 w-6" />
            {{ __("Attendance") }}
        </NavLink>
    </li>
    <li v-if="can('exam-result.index')" class="exam_section">
        <NavLink :href="route('result.index')" :active="route().current('result.index')" class="hover-icons">
            <ClipboardDocumentListIcon class="h-6 w-6 " />
    ...
    <li v-if="can('syllabus.index')" class="syllabus_section">
        <NavLink :href="route('syllabus.index')" :active="route().current('syllabus.*')">
...
import { BellAlertIcon, ClipboardDocumentListIcon, FlagIcon, DocumentDuplicateIcon, ShieldCheckIcon, BookOpenIcon, PlusCircleIcon, VideoCameraIcon, PaperAirplaneIcon, HomeIcon, CalendarIcon, CalendarDaysIcon, PlayCircleIcon, ArrowLeftOnRectangleIcon } from "@heroicons/vue/24/outline";
    ...
    data() {
        return {
            ...
            steps: [
                ...
                {
                    title: this.getOnboardingContent('attendance_section', 'title'),
                    text: this.getOnboardingContent('attendance_section', 'description'),
                    attachTo: {
                ...
                },
                {
                    ...
                    text: this.getOnboardingContent('exam_section', 'description'),
                    attachTo: {
                ...
                {
                    title: this.getOnboardingContent('syllabus_section', 'title'),
                    text: this.getOnboardingContent('syllabus_section', 'description'),
                    attachTo: {
