    ...
    <AppLayout :title="__('Dashboard')">
        ...
        <div class="pb-12">
            ...
            <div class="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6 mb-[25px]">
                <stat-card :title="__('Total Leave')" :route="route('manage-leave.index')" :injected="true">
                    <div class="p-4 flex flex-wrap gap-2">
                        ...
                        <div
                            class="bg-transparent text-blue-700 px-2 py-1.5 text-xs font-bold border border-blue-500 rounded">
                            {{ total_accepted_leaves }} {{ __('Accepted Leaves') }}
                        </div>
                        <div
                            class="bg-transparent text-blue-700 px-2 py-1.5 text-xs font-bold border border-blue-500 rounded">
                            {{ total_rejected_leaves }} {{ __('Rejected Leaves') }}
                        </div>
                    </div>
                </stat-card>
                <stat-card :title="__('Total Subjects')" :value="total_subjects" :label="__('Subjects')"
                    :route="route('my-subject')" />
                <stat-card :title="__('Pending Assignments')" :value="total_assignments" :label="__('Assignments')"
                    :route="route('assignment.index')" />
                <stat-card :title="__('Events Today')" :value="today_events" :label="__('Programs')"
                    ...
                    <div class="card-bottom overflow-hidden">
                        <global-table classes="bg-white">
                            <template #head>
                                ...
                                <th class="py-4 px-5 whitespace-nowrap">{{ __('Deadline') }}</th>
                            </template>
                            <template #body>
                                <template v-if="pending_assignments.length > 0">
                                    <template v-for="assignment in pending_assignments" :key="assignment.id">
                                        <tr
            ...
            <div class="mt-5 bg-white dark:bg-gray-800 rounded-lg xl:col-span-2 xl:row-span-3">
