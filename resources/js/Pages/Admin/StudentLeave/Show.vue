    ...
    <div class="p-6 dark:text-gray-400">
        <div class="w-full">
            <div class="my-2 flex items-center">
                <div>
                    <img class="w-24 h-24 object-cover rounded-lg" :src="leave.user.profile_photo_url" alt="">
                </div>
                <div class="ml-3">
                    <h2 class="dark:text-gray-400 text-lg font-bold mb-0">{{ leave.user.name }}</h2>
            ...
            <h2 class="dark:text-gray-400">
                {{ __('Date') }}: {{ formatTime(leave.start, '') }} - {{ formatTime(leave.end, '') }}
            </h2>
            <h2 class="dark:text-gray-400">
                {{ __('Days') }}: {{ leave.days }}
            </h2>
            ...
            <h2 class="dark:text-gray-400">{{ __('Description') }}: {{ leave.description }}</h2>
            <h2 v-if="leave.status == 'rejected'" class="dark:text-gray-400">{{ __('Cause') }}: {{ leave.rejected_cause  ?? '-'}}</h2>
        </div>
    </div>
</template>
