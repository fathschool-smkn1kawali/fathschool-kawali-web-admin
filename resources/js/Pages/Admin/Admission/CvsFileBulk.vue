<template>
    <form class="mt-2" @submit.prevent="submit()" enctype="multipart/form-data">
        <div class="grid sm:grid-cols-1 md:grid-cols-2 sm:gap-1 md:gap-4">
            <div class="w-full bg-white rounded-lg border border-gray-200 p-4 dark:bg-gray-800 dark:border-gray-700">
                <div class="mb-3">
                    <global-label for="file" value="Select Your File (.csv, .xlsx, .xls)" :required="true" />
                    <global-input ref="file_input"
                        accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel"
                        type="file" @change="picFile($event)" :placeholder="__('Description')" id="name"
                    ...
                    <template v-if="form.file">
                        <progress class="mt-2 rounded-full" v-if="form.progress" :value="form.progress.percentage"
                            max="100">
            ...
            <div class="w-full bg-white rounded-lg border border-gray-200 p-4 dark:bg-gray-800 dark:border-gray-700">
                <div class="mb-3">
                    <div class="mb-3">
                        ...
                        <a href="/files/student.xlsx" download class="inline-flex group relative">
                            <global-button :loading="false" type="button"
                             theme="primary">
                                <ArrowDownTrayIcon class="w-6 h-6 mr-1" />
                                {{ __('Download Example File') }}
                            </global-button>
                            <tool-tip class="text-md" :text="__('Download example file')" />
                        </a>
                        <button @click="visible = true" type="button"
                            ...
                            <tool-tip class="text-md" :text="__('View Example Demo')" />
                        </button>
                    </div>
                </div>
            </div>
        ...
        <global-button :loading="form.processing" type="submit"
            cssClass="w-full mt-3" theme="primary">
            {{ __('Save') }}
        </global-button>
    </form>
    <!-- demo file modal  -->
    <demo-modal maxWidth="w-11/12" :show="visible" @close="closeModal">
        ...
        <template #content>
            <img class="h-full w-full rounded-sm" src="/images/student_sheet.png" alt="">
        </template>

        <template #footer>
            <global-button :loading="false" type="button" @click="closeModal(true)"
                theme="danger">
               {{ __('Close') }}
            </global-button>
        </template>
