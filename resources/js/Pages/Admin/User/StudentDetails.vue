        ...
        <div class="my-2">
            <page-header>
                ...
                <template #content>
                    <div
                        ...
                        <Link :href="route('student.show', student.username)" :data="{ tab: 'details' }"
                        ...
                        <Link :href="route('student.show', student.username)" :data="{ tab: 'attendance' }"
                            :class="request_data.tab == 'attendance' ? 'bg-white dark:bg-gray-500' : ''"
                        ...
                        <Link :href="route('student.show', student.username)" :data="{ tab: 'transactions' }"
                            :class="request_data.tab == 'transactions' ? 'bg-white dark:bg-gray-500' : ''"
                        ...
                        <Link :href="route('student.show', student.username)" :data="{ tab: 'exams' }"
                            :class="request_data.tab == 'exams' ? 'bg-white dark:bg-gray-500' : ''"
                        ...
                        <Link :href="route('student.show', student.username)" :data="{ tab: 'leave-history' }"
            ...
            <div>
                <div v-if="!request_data.tab || request_data.tab == 'details'">
                    <div class="flex flex-col gap-6 lg:items-start lg:flex-row mb-6">
                        <div class="lg:w-1/2 w-full bg-white dark:bg-gray-800 rounded-lg p-6">
                            <div class="flex gap-4 items-center pb-4 border-b-2 dark:border-gray-600">
                                <img :src="student.profile_photo_url" alt="" class="w-20 h-20 rounded-full object-cover">
                                <div>
                                    <h2 class="mb-2 text-lg dark:text-gray-400">
                                        {{ student.name }}
                                    </h2>
                                    <p class="mb-2 text-sm flex gap-1 items-center dark:text-gray-400">
                                        <EnvelopeIcon class="w-6 h-6" />
                                        {{ student.email }}
                                    </p>
                                    <p class="mb-0 text-sm flex gap-1 items-center dark:text-gray-400">
                            ...
                            <div class="grid grid-cols-2 gap-4 pt-4">
                                <div class="flex gap-2">
                                    ...
                                    <div>
                                        <h4 class="text-gray-900 dark:text-gray-500 mb-1">
                                            {{ __('Student ID') }}:
                                        </h4>
                                        <h3 class="text-gray-600 dark:text-gray-400 mb-0 flex">
                                            {{ student?.profile?.student_id ?? '' }}
                                            <template v-if="student?.profile?.student_id ?? false">
                                                <CopyText :text="student.profile.student_id"/>
                                            </template>
                                        </h3>
                                    </div>
                                </div>
                                <div class="flex gap-2">
                                    ...
                                    <div>
                                        ...
                                        <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                ...
                                <div class="flex gap-2">
                                    <div>
                                        <Square3Stack3DIcon class="w-6 h-6 text-blue-600" />
                                    </div>
                                    <div>
                                        <h4 class="text-gray-900 dark:text-gray-500 mb-1">
                                            {{ __('Date of Birth') }}:
                                        ...
                                        <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                            <template v-if="courses.length > 0">
                                                <template v-for="(course, index) in courses" :key="index">
                                                    {{ course }}
                                                    <template v-if="index != courses.length - 1">,</template>
                                                </template>
                                            </template>
                        ...
                        <div class="lg:w-1/2">
                            <div class="w-full bg-white dark:bg-gray-800 rounded-lg">
                                ...
                                <ul class="flex flex-col gap-4 mb-0">
                                    <template v-if="student.documents.length > 0">
                                        <li v-for="(document, index) in student.documents" :key="index" class="flex justify-between items-center p-6">
                                            <div>
                                                <div class="text-base flex gap-1.5 items-center mb-0 dark:text-gray-400">
                                                    <DocumentTextIcon class="w-6 h-6" />
                                                    {{ document.title }}
                                                </div>
                                            </div>
                                            <a :href="document.file_url_format" target="_blank" download="">
                                                <FolderArrowDownIcon class="w-8 h-8 text-blue-600" />
                                            </a>
                                        </li>
                                    </template>
                                    <template v-else>
                            ...
                            <div class="mt-2 w-full bg-white dark:bg-gray-800 rounded-lg">
                                <div class="text-xl dark:text-gray-400 p-6">
                                ...
                                <div class="px-4 flex flex-col gap-4 mb-0">
                                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 pt-4">
                                        <div class="flex gap-2">
                                            <div>
                                                <ArrowLongRightIcon class="w-6 h-6 text-blue-600" />
                                            </div>
                                            <div>
                                                <h4 class="text-gray-900 dark:text-gray-500 mb-1">
                                                    {{ __('Blood Group') }}:
                                                ...
                                                <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                                    {{ student.profile ? student.profile.blood_group : '-' }}
                                                </h3>
                                            </div>
                                        </div>
                                        <div class="flex gap-2">
                                            ...
                                            <div>
                                                ...
                                                <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                                    {{ student.profile ? student.profile.physical_disability : '-' }}
                                                </h3>
                                            </div>
                                        </div>
                                        <div class="flex gap-2">
                                            ...
                                            <div>
                                                ...
                                                <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                                    {{ student.profile ? student.profile.previous_school_name : '-' }}
                                                </h3>
                                            </div>
                                        </div>
                                        <div class="flex gap-2">
                                            ...
                                            <div>
                                                ...
                                                <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                                    {{ student.profile ? student.profile.previous_school_year_of_exist : '-'
                                                    }}
                                                </h3>
                                            </div>
                                        </div>
                                        <div class="flex gap-2">
                                            ...
                                            <div>
                                                ...
                                                <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                                    {{ student.profile ? student.profile.religion : '-' }}
                                                </h3>
                                            </div>
                                        </div>
                                        <div class="flex gap-2">
                                            ...
                                            <div>
                                                ...
                                                <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                                    {{ student.profile ? student.profile.national_identification_number :
                                                        '-' }}
                                                </h3>
                                            </div>
                                        </div>
                                        <div class="flex gap-2">
                                            ...
                                            <div>
                                                ...
                                                <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                                    {{ student.profile ? student.profile.bank_name : '-' }}
                                                </h3>
                                            </div>
                                        </div>
                                        <div class="flex gap-2">
                                            ...
                                            <div>
                                                ...
                                                <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                                    {{ student.profile ? student.profile.bank_account_number : '-' }}
                                                </h3>
                                            </div>
                                        </div>
                                        <div class="flex gap-2">
                                            ...
                                            <div>
                                                ...
                                                <h3 class="text-gray-600 dark:text-gray-400 mb-0">
                                                    {{ student.profile ? student.profile.note : '-' }}
                                                </h3>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="bg-white rounded-lg dark:bg-gray-800 dark:text-gray-400">
                        ...
                        <div class="grid lg:grid-cols-2 sm:grid-cols-2 grid-cols-1 p-6">
                            <template v-if="parents.length">
                                <Link v-for="parent in parents" :key="parent.id" :href="route('parent.show', parent.id)" class="py-4 px-6 bg-white dark:bg-gray-700 rounded-lg">
                                    <div class="flex items-center gap-2 border dark:border-gray-600 p-3">
                                        <div>
                            ...
                            <div v-else class="text-center p-4">{{ __('Nothing Found') }}</div>
                        </div>
                    </div>
                </div>
                                ...
                                <template v-if="transactions.data.length > 0">
                                    <template v-for="transaction in transactions.data" :key="transaction.id">
                                        <tr class="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            ...
                                            <td class="py-5">
                                                {{ transaction.paid_amount }} {{ transaction.currency }}
                                            </td>
                                            <td class="py-5 capitalize">
                                                {{ transaction.status }}
                                            </td>
...
import { DocumentTextIcon, DocumentCheckIcon, FolderArrowDownIcon, BookOpenIcon, EnvelopeIcon, PhoneIcon, HashtagIcon, Square3Stack3DIcon, ArrowLongRightIcon } from "@heroicons/vue/24/outline";
