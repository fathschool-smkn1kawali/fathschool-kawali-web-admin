...
class DataDisplayFathboard extends Controller
{
    ...
    public function sendWhatsAppClass()
    {
        $now = Carbon::now()->format('H:i:s');
        $weekRoutines = Carbon::now()->dayOfWeek;
        $today = Carbon::now()->toDateString();
        ...
        foreach ($gradeData as $grade) {
            foreach ($grade['classes'] as $className) {
                ...
                $classes[] = [
                    ...
                    'total_absent' => $studentsAbsent->count(),
                    'total_leave' => $leaves->count()
                ];
            }
        }

        // Format laporan kehadiran untuk dikirim ke WhatsApp
        $todayFormatted = Carbon::now()->format('d F Y');
        $waMessage = "📊 *Laporan Kehadiran Harian*\n📅 Tanggal: {$todayFormatted}\n\n";

        foreach ($classes as $class) {
            $waMessage .= "📚 *Kelas " . $class['name'] . "*\n";
            $waMessage .= "✅ Hadir: " . $class['total_present'] . "\n";
            $waMessage .= "❌ Absen: " . $class['total_absent'] . "\n";
            $waMessage .= "📜 Izin: " . $class['total_leave'] . "\n\n";
        }
        ...
        // Daftar nomor tujuan
        $adminNumbers = ['62895601096303', '6285220696117']; // Tambahkan nomor lain jika perlu
